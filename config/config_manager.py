...
class ConfigManager:
    ...
    def __init__(self, config_dir: str = None, environment: str = None):
        ...
        logging.info(f"ConfigManager initialized - Environment: {self.environment}, "
    ...
    def _load_configuration(self):
        ...
        base_config = (self._load_config_file("base.yaml") or 
                      self._load_config_file("base.json") or {})
        
        # Load environment-specific configuration
        env_config = (
            self._load_config_file(f"{self.environment}.yaml") or 
            self._load_config_file(f"{self.environment}.json") or 
            {}
        )
        
        # Load local overrides (for development)
        local_config = (self._load_config_file("local.yaml") or 
                       self._load_config_file("local.json") or {})
