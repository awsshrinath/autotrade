...
class ConfigValidator:
    ...
    def _validate_category(
        self, config_dict: Dict, category: str, rules: Dict
    ) -> List[str]:
    ...
    def _validate_business_logic(
        ...
        total_position_limit = (
            config.stock_position_limit
            + config.option_position_limit
            + config.future_position_limit
        )
        if total_position_limit > 1.0:
            warnings.append(
                f"Total position limits ({total_position_limit:.1%}) exceed 100%"
            )
    ...
    def _validate_environment_specific(
        ...
        if config.environment == "production":
            ...
            if config.default_capital < 100000:
                warnings.append(
                    "Production capital below ₹1 lakh may limit trading opportunities"
                )

            if config.max_daily_loss_pct > 5.0:
                warnings.append(
                    "Production daily loss limit above 5% is very aggressive"
                )

            if config.monitoring_interval > 60:
                warnings.append(
                    "Production monitoring interval above 1 minute may miss issues"
                )
    ...
    def _generate_recommendations(
        self, config: TradingConfig, issues: List[str], warnings: List[str]
    ) -> List[str]:
        ...
        if config.default_capital < 50000:
            recommendations.append(
                "Consider increasing capital for better diversification"
            )

        # Risk-based recommendations
        if config.max_daily_loss_pct > 3.0:
            recommendations.append(
                "Consider reducing daily loss limit for better capital preservation"
            )

        if config.margin_utilization_limit > 0.8:
            recommendations.append(
                "High margin utilization increases risk - consider reducing"
            )
    ...
    def compare_configs(
        self, config1: TradingConfig, config2: TradingConfig
    ) -> Dict[str, Any]:
...
def run_config_tests() -> Dict[str, Any]:
    ...
    for environment in ["development", "staging", "production"]:
        try:
            ...
            results[environment] = {"valid": False, "error": str(e)}
