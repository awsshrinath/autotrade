...
def calculate_stop_loss_target(entry_price, direction, atr_value, risk_reward_ratio=2.0):
    """
...
def get_options_recommendation(kite, symbol, direction, strategy='scalp', capital=100000):
    """
    ...
    try:
        ...
        if options_engine.paper_trade:
            ...
            option_chain = asyncio.run(options_engine.analyze_option_chain(kite, symbol, spot_price))
        
        # Select optimal strategy
        recommendation = options_engine.select_optimal_strikes(option_chain, strategy, direction, capital)
...
def calculate_portfolio_exposure(positions):
    ...
    total_long = sum(pos.get('value', 0) for pos in positions if pos.get('quantity', 0) > 0)
    total_short = sum(abs(pos.get('value', 0)) for pos in positions if pos.get('quantity', 0) < 0)
    ...
    if positions:
        ...
        max_position_pct = abs(max_position.get('value', 0)) / total_exposure * 100 if total_exposure > 0 else 0
    ...
    top_positions = sorted(positions, key=lambda x: abs(x.get('value', 0)), reverse=True)[:5]
        ...
        'top_positions': [
            {
                ...
                'percentage': abs(pos.get('value', 0)) / total_exposure * 100 if total_exposure > 0 else 0
            }
