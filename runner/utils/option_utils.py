...
def get_option_chain(kite, symbol, expiry_date=None):
    ...
    if engine.paper_trade:
        ...
        try:
            ...
            import asyncio
            return asyncio.run(engine.analyze_option_chain(kite, symbol, spot_price, expiry_date))
...
def calculate_option_price(spot_price, strike_price, time_to_expiry, 
                          volatility=0.2, interest_rate=0.06, option_type="call"):
    """
...
def analyze_option_strategy(kite, symbol, strategy_type, direction, capital=100000):
    ...
    try:
        ...
        if engine.paper_trade:
            ...
            option_chain = asyncio.run(engine.analyze_option_chain(kite, symbol, spot_price))
        
        # Select optimal strategy
        strategy_result = engine.select_optimal_strikes(option_chain, strategy_type, direction, capital)
        ...
        if strategy_result:
            return {
                ...
                'total_cost': (strategy_result.get('quantity', 1) * 
                              strategy_result.get('primary', {}).market_price if strategy_result.get('primary') else 0),
