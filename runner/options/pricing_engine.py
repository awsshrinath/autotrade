...
class OptionsEngine:
    ...
    def __init__(self, paper_trade: bool = None, risk_free_rate: float = 0.06):
        ...
        if not SCIPY_AVAILABLE and not paper_trade:
            print(
                "WARNING: Running live trading without scipy - falling back to approximations"
            )
        ...
        try:
            if SCIPY_AVAILABLE:
                ...
                def objective(sigma):
                    try:
                        return (
                            self.black_scholes_price(S, K, T, r, sigma, option_type)
                            - market_price
                        )
                    except:
                        return float("inf")
    ...
    async def analyze_option_chain(
        self, kite, symbol: str, spot_price: float, expiry_date: str = None
    ) -> List[OptionData]:
        ...
        try:
            ...
            symbols_list = [
                f"NFO:{opt['tradingsymbol']}" for opt in options[:50]
            ]  # Limit to avoid rate limits
            ltp_data = kite.ltp(symbols_list)

            expiry_datetime = datetime.strptime(expiry_date, "%Y-%m-%d")
            T = max(
                (expiry_datetime.date() - datetime.now().date()).days / 365.0, 0.001
            )
            ...
            for opt in options[:50]:  # Process limited options to avoid timeouts
                ...
                if market_price > 0.5:  # Only process options with meaningful price
                    try:
                        ...
                        liquidity_score = min(
                            market_price / (abs(spot_price - opt["strike"]) + 1), 1.0
                        )
    ...
    def _select_scalp_strikes(
        self, chain: List[OptionData], direction: str, capital: float
    ) -> Dict:
        ...
        if direction == "bullish":
            ...
            calls = [
                opt
                for opt in chain
                if opt.option_type == "CE" and opt.strike <= spot * 1.02
            ]
            candidates = sorted(
                calls,
                key=lambda x: (
            ...
            puts = [
                opt
                for opt in chain
                if opt.option_type == "PE" and opt.strike >= spot * 0.98
            ]
            candidates = sorted(
                puts,
                key=lambda x: (
        ...
        if candidates:
            ...
            max_quantity = int(
                capital * 0.1 / best_option.market_price
            )  # 10% of capital
    ...
    def _select_momentum_strikes(
        ...
        if direction == "bullish":
            ...
            calls = [
                opt
                for opt in chain
                if opt.option_type == "CE" and spot * 1.01 <= opt.strike <= spot * 1.05
            ]
        else:
            ...
            puts = [
                opt
                for opt in chain
                if opt.option_type == "PE" and spot * 0.95 <= opt.strike <= spot * 0.99
            ]
            calls = puts
        ...
        if calls:
            ...
            best_option = max(
                calls, key=lambda x: x.liquidity_score * abs(x.greeks.get("delta", 0))
            )
            max_quantity = int(
                capital * 0.15 / best_option.market_price
            )  # 15% of capital
    ...
    def _select_swing_strikes(
        ...
        if direction == "bullish":
            options = [
                opt
                for opt in chain
                if opt.option_type == "CE" and spot * 0.98 <= opt.strike <= spot * 1.02
            ]
        else:
            options = [
                opt
                for opt in chain
                if opt.option_type == "PE" and spot * 0.98 <= opt.strike <= spot * 1.02
            ]
        ...
        if options:
            ...
            max_quantity = int(
                capital * 0.2 / best_option.market_price
            )  # 20% of capital
    ...
    def _mock_greeks(
        ...
        return GreeksData(
            delta=delta, gamma=gamma, theta=theta, vega=vega, rho=rho, is_mock=True
        )

    def _mock_option_chain(
        self, symbol: str, spot_price: float, expiry_date: str = None
    ) -> List[OptionData]:
        ...
        for strike in strikes:
            for option_type in ["CE", "PE"]:
                ...
                mock_iv = self._mock_implied_volatility(
                    spot_price, strike, T, option_type
                )
    ...
    def _approximate_implied_volatility(
        self,
        market_price: float,
        S: float,
        K: float,
    ...
    def _approximate_greeks(
        ...
        if option_type == "CE":
            delta = (
                0.5 if abs(moneyness - 1) < 0.02 else (0.8 if moneyness > 1 else 0.2)
            )
        else:
            delta = (
                -0.5 if abs(moneyness - 1) < 0.02 else (-0.8 if moneyness < 1 else -0.2)
        ...
        return GreeksData(
            delta=delta, gamma=gamma, theta=theta, vega=vega, rho=rho, is_mock=False
        )
...
def calculate_implied_volatility(
    ...
    engine = OptionsEngine(paper_trade=paper_trade, risk_free_rate=interest_rate)
