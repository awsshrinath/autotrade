...
class PortfolioManager:
    ...
    async def get_real_time_capital(self) -> CapitalAllocation:
        ...
        try:
            ...
            margin_utilization = (
                (used_margin / total_margin * 100) if total_margin > 0 else 0
            )
    ...
    def _get_mock_capital(self) -> CapitalAllocation:
        ...
        self.mock_capital["available_cash"] = max(
            0, self.initial_capital - used_margin + total_position_value
        )

        margin_utilization = (
            (used_margin / self.initial_capital * 100)
    ...
    def calculate_position_size(
        ...
        try:
            ...
            max_position_value = (
                available_capital * self.position_size_limits[asset_type]
            )
            ...
            adjusted_fraction = (
                kelly_fraction * vol_adjustment * conf_adjustment * market_adjustment
            )
            position_value = min(
                max_position_value, available_capital * adjusted_fraction
            )
            ...
            position_info = {
                ...
                "final_allocation_pct": (
                    (quantity * price / available_capital * 100)
            ...
            safe_quantity = int(available_capital * 0.05 / price) if price > 0 else 0
            return {
                "symbol": symbol,
    ...
    def risk_check_before_trade(self, trade_request: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            if capital_data.day_pnl <= -self.max_daily_loss:
                return False, (
                    f"Daily loss limit exceeded: " f"₹{capital_data.day_pnl:,.2f}"
                )

            # 2. Position size check
            new_exposure = quantity * price
            max_single_position = (
                capital_data.total_capital * 0.2
            )  # Max 20% in single position

            if new_exposure > max_single_position:
                return False, (
                    f"Position too large: ₹{new_exposure:,.2f} > "
            ...
            if required_margin > capital_data.available_cash:
                return False, (
                    f"Insufficient cash: Required ₹{required_margin:,.2f}, "
            ...
            max_symbol_exposure = (
                capital_data.total_capital * 0.15
            )  # Max 15% per symbol
    ...
    def calculate_portfolio_metrics(self, days: int = 30) -> RiskMetrics:
        ...
        try:
            ...
            # Calculate returns
            returns = [
                trade.get("return_pct", 0) for trade in trades if "return_pct" in trade
            ]
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        try:
            ...
            for pos_type in ["stock", "option", "future"]:
                type_positions = [
                    p for p in self.positions if p.position_type == pos_type
                ]
                position_summary["positions_by_type"][pos_type] = {
                    "count": len(type_positions),
                    "total_value": sum(
                        [abs(p.quantity * p.current_price) for p in type_positions]
                    ),
            ...
            summary = {
                ...
                "daily_limits": {
                    ...
                    "remaining_loss_capacity": (
                        self.max_daily_loss + capital_data.day_pnl
                    ),
