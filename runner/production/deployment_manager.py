...
class ProductionManager:
    ...
    def _configure_trading_environment(self) -> TradingEnvironment:
        ...
        if self.environment == EnvironmentType.PRODUCTION:
            return TradingEnvironment(
                ...
                position_size_limit=float(
                    os.getenv("POSITION_SIZE_LIMIT", "0.1")
                ),  # 10%
                margin_utilization_limit=float(
                    os.getenv("MARGIN_UTILIZATION_LIMIT", "0.8")
                ),  # 80%
            ...
            return TradingEnvironment(
                ...
                position_size_limit=float(
                    os.getenv("POSITION_SIZE_LIMIT", "0.05")
                ),  # 5%
                margin_utilization_limit=float(
                    os.getenv("MARGIN_UTILIZATION_LIMIT", "0.5")
                ),  # 50%
                ...
                backup_frequency=int(
                    os.getenv("BACKUP_FREQUENCY", "600")
                ),  # 10 minutes
            )
        else:  # DEVELOPMENT
            return TradingEnvironment(
                ...
                position_size_limit=float(
                    os.getenv("POSITION_SIZE_LIMIT", "0.02")
                ),  # 2%
                margin_utilization_limit=float(
                    os.getenv("MARGIN_UTILIZATION_LIMIT", "0.3")
                ),  # 30%
                api_rate_limit=int(os.getenv("API_RATE_LIMIT", "1")),  # 1 req/sec
                monitoring_interval=int(
                    os.getenv("MONITORING_INTERVAL", "120")
                ),  # 2 minutes
                auto_square_off_time=os.getenv("AUTO_SQUARE_OFF_TIME", "15:10"),
                backup_frequency=int(
                    os.getenv("BACKUP_FREQUENCY", "1200")
                ),  # 20 minutes
            )
    ...
    async def comprehensive_health_check(self) -> Dict:
        ...
        if failed_checks or critical_failures:
            overall_status = (
                HealthStatus.CRITICAL if critical_failures else HealthStatus.DEGRADED
            )
        ...
        try:
            ...
            return HealthCheck(
                ...
                details={
                    ...
                    "success_rate": (len(services_status) - len(failed_services))
                    / len(services_status),
                },
                is_critical=True,
            )
    ...
    async def _validate_environment(self) -> Dict:
        ...
        try:
            ...
            return {"step": "environment_validation", "success": False, "error": str(e)}
    ...
    async def _run_startup_health_checks(self) -> Dict:
        ...
        try:
            ...
            return {"step": "startup_health_checks", "success": False, "error": str(e)}
    ...
    async def _validate_credentials(self) -> Dict:
        ...
        try:
            ...
            return {"step": "credentials_validation", "success": False, "error": str(e)}
    ...
    async def _initialize_risk_systems(self) -> Dict:
        ...
        try:
            ...
            portfolio_manager = create_portfolio_manager(
                paper_trade=self.trading_env.paper_trade,
                initial_capital=self.trading_env.max_daily_loss
                * 10,  # Conservative initial capital
            )
            ...
            risk_passed, risk_message = portfolio_manager.risk_check_before_trade(
                test_trade
            )
    ...
    async def _start_monitoring_services(self) -> Dict:
        ...
        try:
            ...
            return {"step": "monitoring_services", "success": False, "error": str(e)}
    ...
    async def _warm_up_caches(self) -> Dict:
        ...
        try:
            ...
            tech_engine = create_technical_engine(
                paper_trade=self.trading_env.paper_trade
            )
            ...
            return {"step": "cache_warmup", "success": False, "error": str(e)}
    ...
    def _handle_circuit_breaker(self):
        ...
        if self.circuit_breaker["failures"] >= self.circuit_breaker["threshold"]:
            ...
            if self.logger:
                self.logger.log_event(
                    "🚨 CIRCUIT BREAKER ACTIVATED - System entering safe mode"
                )
    ...
    def _load_alerts_config(self) -> Dict:
        ...
        return {
            "email_enabled": os.getenv("ALERTS_EMAIL_ENABLED", "false").lower()
            == "true",
            "slack_enabled": os.getenv("ALERTS_SLACK_ENABLED", "false").lower()
            == "true",
