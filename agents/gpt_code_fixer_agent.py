...
def extract_function_block(filepath, function_name):
    ...
    for line in lines:
        ...
        if inside:
            ...
            if line.strip() == "" or (len(line) - len(line.lstrip()) <= indent_level and not line.strip().startswith("def")):
                break
...
def run_code_fixer(log_path="logs/gpt_runner.log"):
    ...
    code_file_path = os.path.join("Tron", trace['file']) if not trace['file'].startswith('/') else trace['file']
    code_block = extract_function_block(code_file_path, trace["function"])
    if not code_block:
        logger.log_event(f"[FixAgent] Could not extract function: {trace['function']} from {trace['file']}")
        return
    ...
    prompt = f"""
You're an AI developer assistant. Analyze the following Python function which caused an error:
...
"""
    ...
    suggestion = ask_gpt(system_prompt="You're an AI developer.", user_prompt=prompt, model=model)

    if suggestion:
        firestore.log_reflection("code-fix", datetime.now().strftime("%Y-%m-%d"), {
            "trace": trace,
